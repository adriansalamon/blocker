...
#+begin_src sh
  ...
  $ cd ~/dns-server/corends

  # Update plugin.cfg and put the line "blocker:github.com/icyflame/blocker" before the "forward:forward" line

  # Build CoreDNS
  $ go get github.com/icyflame/blocker
  $ go generate
  $ make
...
#+begin_src go
  rereplace github.com/icyflame/blocker => <local path to blocker>/blocker
#+end_src
...
#+begin_src sh
  ...
  # Link this plugin locally into CoreDNS
  $ echo "replace github.com/icyflame/blocker => /tmp/dns-server/blocker" >> /tmp/dns-server/coredns/go.mod

  $ cd /tmp/dns-server/coredns
  $ go get github.com/icyflame/blocker
  $ go test -v -count=1 github.com/icyflame/blocker
